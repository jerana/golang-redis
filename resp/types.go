...
// This value is 512MB allowed in Redis.
const (
	MaxBulkSizeLength               = 512 * 1024 * 1024
	MaxBulkSizeAsHumanReadableValue = "512MB"
)

//check if string is a null value
func (r BulkString) IsNull() bool {
	return r.isNullValue
}

// Tag with IDataType methods
...
func NewBulkString(str string) (BulkString, error) {
	...
	if strLen > MaxBulkSizeLength {
		return BulkString{}, errors.New("Cannot allocate a string of length " + strconv.Itoa(strLen) + " because it exceeds max allowed size of " + MaxBulkSizeAsHumanReadableValue)
	}
	return BulkString{
		value:       str,
		isNullValue: false,
